...
<template>
  <Header/>
  <div class="page">
    <div class="expense-card" v-if="userRoleId == 1 || userRoleId == 2">
      <div class="title" v-if="userRoleId == 1">Строевой отдел</div>
      <div class="title" v-if="userRoleId == 2">Мед. служба</div>
...
<script setup>
  ...
  const allowedStatuses = computed(() => {
    if (userRoleId === 1) return [2, 5, 6]
    if (userRoleId === 2) return [3, 4]
    if (userRoleId === 3) {
  ...
  const lockedRowStatuses = computed(() => {
    if (userRoleId === 3) return [3, 4, 5, 6]
    return []
  })
